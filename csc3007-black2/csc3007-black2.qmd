...
merged_data[, c("% Female Legal Coverage", "% Females Unemployed", "% Women in Managerial Positions", "% Female Hourly Earnings")] = scale(merged_data[, c("% Female Legal Coverage", "% Females Unemployed", "% Women in Managerial Positions", "% Female Hourly Earnings")])  
...
#Elbow Method 
fviz_nbclust(merged_data_cols, kmeans, method = "wss")
```
...
set.seed(123)
km.out <- kmeans(merged_data_cols, centers = 4, nstart = 20)
...
tree <- rpart(cluster_id ~., data = train_data, method="class", minsplit=2, minbucket=1)
...
Ten-fold cross validation indicates that the decision tree has a low accuracy of 0.91
...
model <- train(cluster_id ~., data = merged_data, method = "rpart", trControl = ctrl, minsplit=2, minbucket=1)
model
