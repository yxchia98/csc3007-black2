...
# Clean data
indicator_5_1_1_employment_economic_benefits <-
  indicator_5_1_1_employment_economic_benefits |> select(indicator_reference,
                                                         geoAreaName,
                                                         parentName,
                                                         type,
                                                         X,
...
## **5.1.1 vs 5.5.2 Relation Re-expressed**
...
normalised_data_cols <- normalised_data[, c("% Female Legal Coverage", "% Females Unemployed", "% Women in Managerial Positions", "% Female Hourly Earnings")]  
set.seed(123) 
...
set.seed(123)
km.out <- kmeans(normalised_data_cols, centers = 3, nstart = 20)
...
ggpairs(normalised_data,
        columns = c("% Female Legal Coverage", "% Females Unemployed", "% Women in Managerial Positions", "log(% Female Hourly Earnings)"),
        mapping = aes(color = as.factor(Cluster)),
        title = "Pair Plot with K-means Clustering (k = 4)",
        diag = list(continuous = wrap("barDiag", bins = 10, color = "black")),
        lower = list(continuous = wrap("points",
                                       position = position_jitter(height = 1, width = 0.5),
                                       fill = "black")),
...
tree <- rpart(cluster_id ~., data = train_data, method="class", minsplit=3, minbucket=1)
...
model <- train(cluster_id ~., data = merged_data, method = "rpart", trControl = ctrl, minsplit=3, minbucket=1)
model
...
Ten-fold cross validation indicates that the decision tree has a low accuracy of "Fill in"
