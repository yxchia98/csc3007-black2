...
# Step1: Import Data
happy_index <- read_excel("data/happy-planet-index-2006-2020-public-data-set.xlsx",
                          sheet = 2,
                          skip = 7)
happy_index <- select(
...
tree <- rpart(cluster_id ~., data = train_data, method="class", minsplit = 6, minbucket = 2)
train_data$predicted <- predict(tree, type = "class")

# Plot the decision tree with a transparent background
par(bg = NA)
rpart.plot(tree, digits=-3, box.palette = list("#f8766d", "#00bfc4", "#56B4E9"))
```
:::
...
model <- train(cluster_id ~., data = merged_data, method = "rpart", trControl = ctrl, minsplit = 6, minbucket = 1)
