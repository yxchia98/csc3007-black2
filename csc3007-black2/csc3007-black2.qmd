...
merged_data_cols <- merged_data[, c("% Female Legal Coverage", "% Females Unemployed", "% Women in Managerial Positions", "% Female Hourly Earnings")]  
set.seed(123) 
...
for (k in 1:10) {
  kmeans_result <- kmeans(merged_data_cols, centers = k)
...
set.seed(123)
km.out <- kmeans(merged_data_cols, centers = 3, nstart = 20)

# Add cluster information to the merged_data dataframe
merged_data$Cluster <- km.out$cluster
...
merged_data <- select(
  merged_data, ISO3 = "ISO3", cluster_id = "Cluster",
) |> drop_na()
...
The decision tree does not assign any country to cluster 2 (*Fill in*).
